-- Add is_public opt-in column to relays (default false — no relay is public until opted in)
ALTER TABLE public.relays ADD COLUMN IF NOT EXISTS is_public boolean NOT NULL DEFAULT false;

-- Update the public directory function to only return opted-in relays
...
-- Allow anon to see is_public column (already covered by existing "Anyone can view all relays" policy)
-- But update the policy to only expose public relays to anon users
DROP POLICY IF EXISTS "Anyone can view all relays" ON public.relays;
CREATE POLICY "Anyone can view public relays" ON public.relays
  FOR SELECT TO anon USING (is_public = true);
CREATE POLICY "Authenticated can view all relays" ON public.relays
